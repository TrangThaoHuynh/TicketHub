...
def my_tickets():
    ...
    try:
        ...
        for row in bookings.items:
            paid = (str(row.booking_status or '').upper() == 'SUCCESS') or (str(row.payment_status or '').upper() == 'SUCCESS')
            orders.append(
                {
        ...
        return render_template('my_tickets.html', orders=orders, bookings=bookings)
    except ProgrammingError:
        # If DB schema isn't available yet, fall back to old ticket list view.
...
def booking_detail(booking_id: int):
    ...
    try:
        ...
        paid = (str(getattr(booking, 'status', '')).upper() == 'SUCCESS') or (str(getattr(payment, 'status', '')).upper() == 'SUCCESS')
        ...
        groups = {}
        total = 0
        for t, tt, _ev in ticket_rows:
            code = t.ticketCode or t.id
            if t.price is not None:
                total += float(t.price)

            group_key = tt.name or f"Vé loại {tt.id}"
        ...
        abort(500, description='Database chưa có bảng cần thiết (Booking/Ticket/...). Hãy chạy script tạo database trước.')