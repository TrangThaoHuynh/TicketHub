...
def _build_hash_data(params: Mapping[str, Any]) -> str:
    """
    Build canonical VNPay query string used BOTH for:
    - signing request
    - verifying response

    Keep quote_plus() because your local test showed it matches the real signature.
    """
    filtered = {
        key: value
        for key, value in _normalize_params(params).items()
...
def create_secure_hash_from_string(hash_data: str, hash_secret: str | None = None) -> str:
    config = get_vnpay_config()
    secret = (hash_secret or config["vnp_hash_secret"]).strip()
    ...
    if has_app_context():
        ...
        print(f"[VNPAY] build hash_data={hash_data}", flush=True)
...
def create_secure_hash(params: Mapping[str, Any], hash_secret: str | None = None) -> str:
    ...
    """
    hash_data = _build_hash_data(params)
    return create_secure_hash_from_string(hash_data, hash_secret)
...
def build_payment_url(
    ...
    # Build ONCE, use the SAME string for both URL and signature.
    query_without_hash = _build_hash_data(params)
    secure_hash = create_secure_hash_from_string(query_without_hash, config["vnp_hash_secret"])
    query_with_hash_type = f"{query_without_hash}&vnp_SecureHashType=SHA512"
    payment_url = f"{config['vnp_url']}?{query_with_hash_type}&vnp_SecureHash={secure_hash}"
...
def verify_return_data(query_params: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    # IMPORTANT: remove signature fields before rebuilding the canonical string.
    normalized_for_hash = dict(normalized)
    normalized_for_hash.pop("vnp_SecureHash", None)
    normalized_for_hash.pop("vnp_SecureHashType", None)

    expected_hash = create_secure_hash(normalized_for_hash)
    valid_signature = bool(secure_hash) and hmac.compare_digest(
        secure_hash.lower(),
        expected_hash.lower(),
    )
...
def build_mock_return_url(txn_ref: str, amount: float | int, success: bool = True) -> str:
    ...
    query = _build_hash_data(params)
    secure_hash = create_secure_hash_from_string(query, get_vnpay_config()["vnp_hash_secret"])
    query_with_hash_type = f"{query}&vnp_SecureHashType=SHA512"
