...
def _build_hash_data_from_raw_query(query_string: str) -> str:
	if not query_string:
		return ""
...
def _build_query_string(params: Mapping[str, Any]) -> str:
    return _build_hash_data(params)
...
def verify_return_data(query_params: Mapping[str, Any], raw_query: bytes | str | None = None) -> Dict[str, Any]:
	...
	if not valid_signature and raw_query:
		raw_query_text = (
			raw_query.decode("utf-8")
			if isinstance(raw_query, (bytes, bytearray))
			else str(raw_query)
		)
		raw_hash_data = _build_hash_data_from_raw_query(raw_query_text)
		if raw_hash_data:
			config = get_vnpay_config()
			raw_expected_hash = hmac.new(
