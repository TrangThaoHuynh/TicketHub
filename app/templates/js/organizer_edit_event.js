document.addEventListener("DOMContentLoaded", function () {
	...
	const initialEventDataScript = document.getElementById("initialEditEventData");
	let initialEventData = {};
	if (initialEventDataScript) {
		try {
			initialEventData = JSON.parse(initialEventDataScript.textContent || "{}");
		} catch (e) {
			initialEventData = {};
		}
	}

	function normalizeInitialTicket(rawTicket) {
		const parsedTicketId = Number(rawTicket && rawTicket.id);
		const parsedPrice = Number(rawTicket && rawTicket.price);
		const parsedQuantity = Number(rawTicket && rawTicket.quantity);

		return {
			id: Number.isInteger(parsedTicketId) && parsedTicketId > 0 ? parsedTicketId : null,
			name: String((rawTicket && rawTicket.name) || "").trim(),
			isFree: Boolean(rawTicket && rawTicket.isFree) || parsedPrice <= 0,
			price: Number.isFinite(parsedPrice) ? String(parsedPrice) : "0",
			quantity: Number.isFinite(parsedQuantity) && parsedQuantity > 0 ? String(Math.floor(parsedQuantity)) : "1",
	...
	function cloneTicketPayload(ticket) {
		return {
			id: ticket.id,
			name: ticket.name,
	...
	const initialTicketTypes = Array.isArray(initialEventData.tickets)
		? initialEventData.tickets.map(normalizeInitialTicket)
		: [];
	const initialEventDescriptionHtml = String(initialEventData.description || "").trim();
	const initialEventImageUrl = String(initialEventData.imageUrl || "").trim();
	const initialEventImageName = String(initialEventData.imageName || "event-image.jpg").trim() || "event-image.jpg";
	...
	if (descriptionEditorEl && typeof window.Quill !== "undefined") {
		...
		if (initialEventDescriptionHtml) {
			descriptionEditor.clipboard.dangerouslyPasteHTML(initialEventDescriptionHtml);
		}
	}

	if (descriptionHiddenInput) {
		descriptionHiddenInput.value = initialEventDescriptionHtml;
	}
	...
	const ticketTypes = initialTicketTypes.map(cloneTicketPayload);
	...
	let ticketAdjustLocked = false;

	function normalizeStatus(status) {
		const text = String(status || "").trim().toUpperCase();
		return text === "PUBLISHED" ? "PUBLISHED" : "PENDING";
	}

	function getSelectedEventStatus() {
		const checked = document.querySelector('input[name="eventStatus"]:checked');
		if (checked) {
			return normalizeStatus(checked.value);
		}
		return normalizeStatus(initialEventData.eventStatus);
	}
	...
	function setSuggestButtonLoading(isLoading) {
		...
		suggestTicketPriceBtn.disabled = Boolean(isLoading) || ticketAdjustLocked;
		suggestTicketPriceBtn.textContent = isLoading ? "Đang gợi ý..." : "Gợi ý giá";
	}

	function applyTicketAdjustLock(status) {
		ticketAdjustLocked = normalizeStatus(status) !== "PENDING";
		if (openTicketFormBtn) {
			openTicketFormBtn.disabled = ticketAdjustLocked;
		}

		if (ticketAdjustLocked) {
			closeTicketForm();
		}

		if (ticketFreeInput && ticketAdjustLocked) {
			ticketFreeInput.checked = Boolean(ticketFreeInput.checked);
		}

		if (suggestTicketPriceBtn) {
			suggestTicketPriceBtn.disabled = ticketAdjustLocked || (ticketFreeInput && ticketFreeInput.checked);
	...
	function updateOpenImageEditorButtonState() {
		if (openImageEditorBtn) {
			openImageEditorBtn.disabled = !sourceEventImageFile && !initialEventImageUrl;
		}
	}
	...
	function loadFileFromImageUrl(imageUrl) {
		if (!imageUrl) {
		...
		return fetch(imageUrl)
			...
			.then(function (blob) {
				const mimeType = normalizeImageMimeType(blob.type || "image/jpeg");
				return new File([blob], initialEventImageName, {
					type: mimeType,
	...
	function restoreCommittedUploadSelection() {
		...
		if (initialEventImageUrl) {
			uploadInput.value = "";
			setUploadPreviewFromDataUrl(initialEventImageUrl);
			return;
		}
	...
	function openTicketFormForEdit(index) {
		...
		ticketQuantityInput.value = ticket.quantity != null ? String(ticket.quantity) : "";
		...
		if (isFree) {
			...
			ticketPriceInput.value = ticket.price != null ? String(ticket.price) : "";
		}

		if (suggestTicketPriceBtn) {
			suggestTicketPriceBtn.disabled = ticketAdjustLocked || isFree;
			suggestTicketPriceBtn.textContent = "Gợi ý giá";
		}
	...
	function renderTicketTypes() {
		...
		const isLocked = Boolean(ticketAdjustLocked);
		const actionDisabledAttr = isLocked ? " disabled" : "";
	...
	function resetTicketForm() {
		...
		if (suggestTicketPriceBtn) {
			suggestTicketPriceBtn.disabled = ticketAdjustLocked;
			suggestTicketPriceBtn.textContent = "Gợi ý giá";
		}
	}

	function buildSuggestPricePayload() {
		const eventId = Number(initialEventData.eventId);
	...
	function validateSuggestPriceInputs(data) {
		if (ticketAdjustLocked) {
			window.alert("Chỉ có thể gợi ý giá vé khi sự kiện đang xử lý (PENDING).");
			return false;
		}
		if (!data.eventTypeId) {
	...
	if (openImageEditorBtn) {
		openImageEditorBtn.addEventListener("click", function () {
			...
			if (openImageEditorBtn) {
				openImageEditorBtn.disabled = true;
			}

			loadFileFromImageUrl(initialEventImageUrl)
	...
	openTicketFormBtn.addEventListener("click", function () {
		if (ticketAdjustLocked) {
			window.alert("Sự kiện đã công khai nên không thể điều chỉnh loại vé.");
			return;
		}
	...
	ticketFreeInput.addEventListener("change", function () {
		if (ticketFreeInput.checked) {
			...
			ticketPriceInput.disabled = ticketAdjustLocked;
			if (suggestTicketPriceBtn) {
				suggestTicketPriceBtn.disabled = ticketAdjustLocked;
			}
		}
	...
	ticketTypeForm.addEventListener("submit", function (e) {
		...
		const editingTicket = editingTicketIndex >= 0 ? ticketTypes[editingTicketIndex] : null;
		const rawTicketPrice = (ticketPriceInput.value || "").trim();
		const rawTicketQuantity = (ticketQuantityInput.value || "").trim();

		const payload = {
			id: editingTicket && editingTicket.id ? editingTicket.id : null,
	...
	const eventStatusInputs = Array.from(document.querySelectorAll('input[name="eventStatus"]'));
	eventStatusInputs.forEach(function (input) {
		input.addEventListener("change", function () {
			applyTicketAdjustLock(getSelectedEventStatus());
		});
	});
	...
	eventForm.addEventListener("submit", function (e) {
		...
		if (!ticketTypes.length) {
			e.preventDefault();
			window.alert("Vui lòng tạo ít nhất một loại vé trước khi cập nhật sự kiện.");
			return;
		}
		...
		if (imageEditorModal && !imageEditorModal.classList.contains("is-hidden")) {
			e.preventDefault();
			window.alert("Vui lòng nhấn Đồng ý hoặc Hủy trên form chỉnh sửa ảnh trước khi cập nhật sự kiện.");
			return;
		}
	...
	eventForm.addEventListener("reset", function () {
		window.setTimeout(function () {
			...
			initialTicketTypes.forEach(function (ticket) {
				ticketTypes.push(cloneTicketPayload(ticket));
			});
			renderTicketTypes();
			closeTicketForm();
