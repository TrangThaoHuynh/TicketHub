...
function getPageContext() {
  ...
  return {
    ...
    checkoutUrl: `/events/${parseInt(root.dataset.eventId || "0", 10)}/checkout`,
  };
}
  ...
  for (let idx = 1; idx <= item.quantity; idx += 1) {
    ...
    rows.push(
      '<div class="ticket-holder" data-holder-key="' +
        ...
        idx +
        '">' +
        '<span class="ticket-holder__label">Vé ' +
        idx +
        "</span>" +
        ...
        '<label for="name-' +
        inputBase +
        '">Họ và tên người mua</label>' +
        '<div class="input-with-icon">' +
        '<input id="name-' +
        inputBase +
        '" class="js-holder-name" type="text" placeholder="Nguyễn Văn A">' +
        ...
        '<input id="phone-' +
        inputBase +
        '" class="js-holder-phone" type="tel" placeholder="0912345678">' +
        '<i class="fa-solid fa-check field-valid-icon"></i>' +
        "</div>" +
  ...
  accordion.innerHTML = order.tickets
    .map((item, index) => {
      ...
      return (
        ...
        "</button>" +
        '<div class="ticket-group__body">' +
        renderTicketHolderRows(item, requireFace) +
        "</div>" +
        "</article>"
      );
    })
...
function showEmptyState(ctx) {
  ...
  if (summaryList)
    summaryList.innerHTML =
      '<p class="summary-empty">Bạn chưa có vé tạm giữ cho sự kiện này.</p>';
  ...
  if (paymentHint) {
    ...
    paymentHint.textContent =
      "Vui lòng quay lại để chọn loại vé trước khi thanh toán.";
  }
  ...
  holders.forEach((holder) => {
    ...
    if (phoneInput) {
      ...
      phoneInput.classList.toggle(
        "is-invalid",
        !validPhone && String(phoneInput.value || "").trim().length > 0,
      );
    }
    ...
    if (faceInput)
      faceInput.classList.toggle("is-invalid", requireFace && !validFace);
...
function updatePaymentState(requireFace) {
  ...
  payButton.disabled = !formValid || payButton.dataset.busy === "1";
...
function readFileAsDataUrl(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(new Error("Không thể đọc ảnh khuôn mặt."));
    reader.readAsDataURL(file);
  });
}

async function buildCheckoutPayload(order, requireFace) {
  const payloadTickets = [];

  for (const item of order.tickets) {
    const ticketBlock = document.querySelector(
      `[data-ticket-type-block="${item.ticketTypeId}"]`,
    );
    if (!ticketBlock) continue;

    const holderNodes = Array.from(
      ticketBlock.querySelectorAll(".ticket-holder"),
    );
    const holders = [];

    for (const holderNode of holderNodes) {
      const nameInput = holderNode.querySelector(".js-holder-name");
      const phoneInput = holderNode.querySelector(".js-holder-phone");
      const faceInput = holderNode.querySelector(".js-holder-face");

      const holder = {
        fullName: String(nameInput ? nameInput.value : "").trim(),
        phoneNumber: String(phoneInput ? phoneInput.value : "").trim(),
      };

      const file =
        faceInput && faceInput.files && faceInput.files[0]
          ? faceInput.files[0]
          : null;
      if (file) {
        holder.faceImageBase64 = await readFileAsDataUrl(file);
      } else if (requireFace) {
        throw new Error("Vui lòng tải ảnh khuôn mặt cho tất cả vé.");
      }
  ...
  return { tickets: payloadTickets };
}

function setPayButtonBusy(isBusy) {
  const payButton = document.getElementById("payButton");
  if (!payButton) return;

  payButton.dataset.busy = isBusy ? "1" : "0";
  payButton.disabled = isBusy;
  payButton.innerHTML = isBusy
    ? "Đang xử lý..."
    : 'Thanh toán <i class="fa-solid fa-arrow-right-long"></i>';
}
...
function bindPaymentAction(order, ctx) {
  ...
  payButton.addEventListener("click", async function () {
    ...
    try {
      const payload = await buildCheckoutPayload(order, ctx.requireFace);
      ...
      alert(error.message || "Không thể tiếp tục thanh toán.");
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const ticketMetaMap = getTicketMetaMap(
    parseJsonScript("confirmTicketTypesData", []),
  );
